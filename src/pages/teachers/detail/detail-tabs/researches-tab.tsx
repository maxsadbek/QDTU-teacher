...
import type { Research } from "@/features/research/research.type";
...
export function ResearchesTab({
  ...
  const columns: ColumnDef<Research>[] = [
    ...
    },
    {
      ...
      header: "A'zolik turi",
      cell: ({ row }) => {
        const type = row.getValue("memberEnum") as Research["memberEnum"];
        return (
          <Badge
            className={
              type === "XALQARO"
      ...
      cell: ({ row }) => (
        ...
        >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 text-[12px] font-semibold px-2.5 py-1.5 rounded-md transition-colors cursor-pointer"
          >
            <Pencil className="size-3" /> Tahrirlash
          </button>

          {/* O'CHIRISH TUGMASI (POPOVER BILAN) */}
          <ConfirmPopover 
            onConfirm={() => deleteResearch(row.original.id)} 
            message="Ushbu tadqiqotni o'chirishni tasdiqlaysizmi?"
          >
            <button 
              type="button" 
              disabled={isDeleting}
              className="inline-flex items-center gap-1.5 bg-red-50 text-red-600 hover:bg-red-100 text-[12px] font-semibold px-2.5 py-1.5 rounded-md transition-colors cursor-pointer disabled:opacity-50"
            >
              <Trash2 className="size-3" /> 
              {isDeleting ? "O'chmoqda..." : "O'chirish"}
            </button>
          </ConfirmPopover>
        </div>
      ),
